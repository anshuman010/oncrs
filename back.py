...
def connect_db():
    try:  # error handling for databse connection
        global con
        con = mysql.connector.connect(host='localhost', user='root', passwd='837607')
    except:
        warning = "Enter your Database host, user and password in command line"
        tkinter.messagebox.showwarning(title="Db error", message=warning)
        h = input("Enter host: ")
        u = input("Enter username: ")
        p = input("Enter password: ")
        con = mysql.connector.connect(host=h, user=u, password=p)
    ...
    def create_db():
        try:  # error handling if databse is there
            cur.execute("create database ocr")
        except:
...
def create():
    con = mysql.connector.connect(host='localhost', user='root', passwd='837607',database="ocr")
    cur=con.cursor()
...
def create_acc():
    ...
    def submit():
        if fname.get()=="" or lname.get()=="" or email.get() == "" or password.get()=="" :
            ...
            conn=mysql.connector.connect(host='localhost', user='root', passwd='837607', database='OCR')
            cur1=con.cursor()
            ...
            conn.commit()
            if flag==0:
                conn1 = mysql.connector.connect(host='localhost', user='root', passwd='837607', database='OCR')
                cur=conn.cursor()
                cur2=cur.execute("insert into createaccount values(%s,%s,%s,%s)",(fname.get(),lname.get(),email.get(),password.get()))
                conn.commit()
